class Room(object):
    ...
    def go(self,direction):
        return self.path.get(direction)
...
"""
Lucky for you they made you learn Gothon insults in the academy. You tell the one Gothon joke 
you know:Lbhe zbgure vf fb sng, jura fur fvgf nebhaq gur ubhfr, fur fvgf nebhaq gur ubhfr.
The Gothon stops, tries not to laugh, then busts out laughing and can't move.While he's laughing 
you run up and shoot him square in the head putting him down, then jump through the Weapon Armory door.

You do a dive roll into the Weapon Armory, crouch and scan the room for more Gothons that 
might be hiding.  It's dead quiet, too quiet. You stand up and run to the far side of the room 
and find the neutron bomb in its container.  There's a keypad lock on the box and you need the 
code to get the bomb out.  If you get the code wrong 10 times then the lock closes forever and 
you can't get the bomb.  The code is 4 digits.
""")
...
blaster at it. You then jump back through the door, punch the close button and blast the lock so the 
Gothons can't get out. Now that the bomb is placed you run to the escape pod to get off this tin can.
...
chamber with the escape pods, and ow need to pick one to take.  Some of them could be damaged but you don't
...
As it flies to the planet, you look back and see your ship implode then explode like a bright star, taking out 
the Gothon ship at the same time.  You won!
""")

the_end_loser=Room("You Lost",
"""
You jump into a random pod and hit the eject button.The pod escapes out into the void of space, then implodes 
...
class LaserDeath(Room):
    def __init__(self,name,description,password,count):
        super(LaserDeath,self).__init__(name,description)
        self.count=count
        #self.description+=",tries left:%d."%count
    ...
    def go(self,guess):
        ...
        if guess!=self.password:
            if self.count <=1:
                ...
                #print("You have %d times left" %count)
                #copy itself but with --count
                #loop=LaserDeath(self.name,self.description,self.password,self.count-1)
                #loop.add_paths(self.paths)
                #self.count-=1
                #self.description[-2]='%d'%self.count
                return self
        else:
            return self.path.get(guess)
...
laser_death=LaserDeath("keypad","try passwords",'0132',9)

laser_death.add_paths({
    laser_death.password: the_bridge,
    '*':generic_death
})
...
look_up={
    ...
    'keypad': laser_death,
    'You Won':the_end_winner,
    'You Lost':the_end_loser
}
